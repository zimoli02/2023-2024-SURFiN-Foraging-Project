...
def main():
    ...
    for session, j in zip(list(long_sessions.itertuples()), range(len(long_sessions))):
        if j != 3:
            ...
            np.save(title+"Transition.npy", transition_mat[index].T[index].T)
            ...
            np.save(title+"States.npy", states)
            ...
            for i in range(N):
                ...
                axs[i].set_xlabel('X')
                axs[i].set_ylabel('Y')
            plt.savefig('/nfs/nhome/live/zimol/ProjectAeon/aeon_mecha/images/HMM/'+title+'Position.png')
            ...
            plt.savefig('/nfs/nhome/live/zimol/ProjectAeon/aeon_mecha/images/HMM/'+title+'Data.png')
