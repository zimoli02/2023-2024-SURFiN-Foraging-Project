...
def FindModels():
    ...
    for N in range(3,17):
        ...
        title = 'ShortSession1'
        mouse_pos = pd.read_parquet('ShortSession1_mousepos.parquet', engine='pyarrow')
    ...
    np.save('../Figures/LogLikelihood.npy', LogLikelihood)

def DisplayLikelihood():
    N = np.arange(3,17,1)
    LogLikelihood = np.load('../Figures/LogLikelihood.npy', allow_pickle=True)
...
def FitExampleSession(id, n):
    ...
    mouse_pos = pd.read_parquet(title + '_mousepos.parquet', engine='pyarrow')
    ...
    plt.tight_layout()
    plt.savefig('../Figures/Results/ExampleParameters.png')
    plt.show()
...
def Pellets(id, n):
    ...
    try:
        PELLET = pd.read_parquet('PELLET.parquet', engine='pyarrow')
        ...
        PELLET.to_parquet('PELLET.parquet', engine='pyarrow')
...
def DataInArena(id, n, denoise):
    ...
    plt.tight_layout()
    plt.savefig('../Figures/Results/ExampleParameters_Arena.png')
    plt.show()
...
def DirectionInArena(id, n, denoise):
    ...
    plt.tight_layout()
    plt.savefig('../Figures/Results/ExampleDirection_Arena.png')
    plt.show()
