...
def FindModels():
    ...
    for N in range(3,17):
        ...
        width = 0.4
        axs.bar(np.arange(N)-width/2, mean_speed, yerr=var_speed, width = width, capsize=5, color = 'blue', label = 'Speed')
        axs.bar(np.arange(N)+width/2, mean_acce, yerr=var_acce, width = width, capsize=5, color = 'orange', label = 'Acce')
        axs.set_xticks(range(0, N), [str(i) for i in range(N)])
    ...
    np.save('../FigureData/LogLikelihood.npy', LogLikelihood)
...
def DisplayLikelihood():
    ...
    plt.tight_layout()
    plt.savefig('../FigureResults/LogLikelihood.png')
    plt.show()

def FitExampleSession(id, n):
    title = 'ShortSession' + str(id)
    mouse_pos = pd.read_parquet('../FigureData/' + title + '_mousepos.parquet', engine='pyarrow')
    ...
    plt.tight_layout()
    plt.savefig('../FigureResults/ExampleParameters.png')
    plt.show()
    
    np.save('../FigureResults/ExampleTransM.npy', transition_mat[speed_index].T[speed_index].T)
    ...
    np.save('../FigureData/States.npy', states)
...
def DisplayExampleSessionTransM(id, n, raw = True, denoise = True):
    if raw:
        TransM = np.load('../FigureData/ExampleTransM.npy', allow_pickle=True)
    else:
        TransM = ManualTransM(id, n, denoise = denoise)
    HMM.PlotTransition(TransM, title = '../FigureResults/ExampleTransM.png')
...
def DisplayExampleSessionPosition(id, n, denoise = True):
    ...
    plt.tight_layout()
    plt.savefig('../FigureResults/ExamplePositionMap.png')
    plt.show()
...
def DisplayExampleSessionPosition_Speed(id, n, denoise = True):
    ...
    plt.tight_layout()
    plt.savefig('../FigureResults/ExamplePositionMap_Speed.png')
    plt.show()
...
def DisplayExampleSessionPosition_Acce(id, n, denoise = True):
    ...
    plt.tight_layout()
    plt.savefig('../FigureResults/ExamplePositionMap_Acce.png')
    plt.show()
...
def Pellets(id, n):
    ...
    try:
        ...
        PELLET.to_parquet('../FigureData/PELLET.parquet', engine='pyarrow')
    ...
    plt.savefig('../FigureResults/ExamplePelletStates.png')
    plt.show()
...
def DisplayExampleSessionState(id, n, denoise = True):
    ...
    HMM.PlotStates(mouse_pos['states'].to_numpy(), mouse_pos, n, title = '../FigureResults/ExampleStates.png')
...
def DataInArena(id, n, denoise):
    ...
    plt.tight_layout()
    plt.savefig('../FigureResults/ExampleParameters_Arena.png')
    plt.show()
